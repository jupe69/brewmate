...
## Project Overview
BrewMate is a native macOS GUI application for Homebrew package management. Built with SwiftUI for macOS 14.0+ (Sonoma), it provides a clean, native interface for managing formulae, casks, services, taps, and more.
...
```
BrewMate/
├── BrewMateApp.swift              # App entry point, menus, settings
├── Info.plist                      # App configuration
├── BrewMate.entitlements          # Sandbox disabled, hardened runtime
...
├── State/
│   └── AppState.swift             # @Observable global state, 14 sidebar sections
├── Views/
│   ├── ContentView.swift          # Main 3-column NavigationSplitView
│   ├── Sidebar/
│   │   └── SidebarView.swift      # Sidebar with 14 sections
│   ├── Search/
...
- Favorites (Set<String>) - stored in ~/Library/Application Support/BrewMate/
- Notes (Dictionary<String, String>) - per-package notes
- History ([HistoryEntry]) - install/uninstall/upgrade log (max 500 entries)

### State/AppState.swift
Global observable state with 14 sidebar sections:
1. Search, 2. Installed, 3. Formulae, 4. Casks, 5. Updates
6. Favorites, 7. Pinned, 8. Taps, 9. Services, 10. Brewfile
11. Diagnostics, 12. Cleanup, 13. Quarantine, 14. History
...
xcodebuild -project BrewMate.xcodeproj -scheme BrewMate -configuration Debug build

# Clean and rebuild
xcodebuild -project BrewMate.xcodeproj -scheme BrewMate clean build
...
- **Bundle ID**: com.multimodalsolutions.brewmate
