...
struct WidgetData: Codable {
    ...
    static func load() -> WidgetData {
        guard let containerURL = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: "group.com.multimodalsolutions.brewmate"
        ) else {
            return .empty
        }
...
struct BrewMateWidgetEntryView: View {
...
struct BrewMateWidget: Widget {
    let kind: String = "BrewMateWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: Provider()) { entry in
            BrewMateWidgetEntryView(entry: entry)
        }
        .configurationDisplayName("BrewMate Updates")
